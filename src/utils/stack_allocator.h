...
StackAllocator *stackAllocatorInit(void* stackBase);
...
void* stackAllocatorStaticMalloc(size_t size);
void stackAllocatorStaticFree(void *p);
void *stackAllocatorStaticRealloc(void *p, size_t size);
...
void *stackAllocatorStaticRealloc(void *p, size_t size)
{
	return stackAllocatorRealloc(stackAllocator, p, size);
}

void* stackAllocatorStaticMalloc(size_t size)
{
	return stackAllocatorAlloc(stackAllocator, size);
}
...
// SafeMemory starts from 0xFFFF800000000000 + last addres of the program
StackAllocator *stackAllocatorCreate(void* stackBase)
{
...
{
    stackAllocator = stackAllocatorCreate(((uint64_t *)(0xFFFF800000F00000)));
}
