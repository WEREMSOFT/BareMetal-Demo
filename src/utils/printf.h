...
int printf(const char *format, ...) {
    ...
    int num_written = snprintf(buffer, sizeof(buffer), format, args);

    va_end(args);

    if (num_written > 0) {
        size_t to_write = (num_written < sizeof(buffer)) ? num_written : sizeof(buffer) - 1;
        b_output(buffer, to_write);
    }
